...
tarif,FIO,traf,balance,duration
        ...
        try:
            ...
            return False
        k={}
        m=re.search(u'alt="Тарифный план"></td>.*?<td>&nbsp;<b class=body>(.*?)<',html,16)
        if m:
            s=m.group(1)
            s=s.strip()
            k['tarif']=s
    ...
    def _parsesessions(self,row,title):
        ...
        try:
            ...
            self._SetLastError(str(e))
            return None
    def GetLog(self,period='current',fromfile=None,encoding='cp1251'):
...
%s
%s\
        '''%(info.get('FIO'),info.get('tarif'),info.get('balance'),traf,duration))
        return True
